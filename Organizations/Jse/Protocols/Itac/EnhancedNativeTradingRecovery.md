## Itac Enhanced Native Trading Recovery: Enhanced Native Trading Recovery

Recovery channel of the Enhanced Native Trading gateway, replaying server initiated messages a client missed on the Real-Time channel for the Derivative and Bond Markets of the Johannesburg Stock Exchange.

### Overview

Each Enhanced Native Trading server exposes two channels. Orders are submitted on the Real-Time channel; the Recovery channel exists so a client that was disconnected can retrieve the server initiated messages generated while it was away, which are not resent on the Real-Time channel when it reconnects. A client must already hold a Real-Time session before logging in to Recovery, using the same Interface User CompID and password, and any New Password supplied on the Recovery Logon is ignored.
...
- **Server initiated messages only** - Replays Execution Report, Order Cancel Reject, Order Mass Cancel Report, Security Definition Response, News and Business Reject; order entry messages never appear on this channel
